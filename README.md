...
- **ガード**は相手と逆方向に入力。**通常技はガードすればダメージ0**で、削られるのは飛び道具などの必殺技だけです（本家スト2と同じ）。`↓` を一緒に入れると**しゃがみガード**になり、姿勢も専用の絵に切り替わります（Ver.9）
...
- **ヨガファイヤー（Ver.8）**: ダルシムの**伸縮する炎**（`↓` を溜めて `C` を押しながら**前**）。弾は飛ばさず、口から伸びて引っ込むまでの炎そのものが当たり判定です。伸びきったところで**343px**（キック95pxの3.6倍）に届きますが、その距離が出るのは6フレームだけ。全体60Fと長いので、外すと手痛く反撃されます。当てると相手を**128px（体2.5個分）吹っ飛ばします**（通常技は24px）。ただし吹っ飛ばしすぎると近寄れなくなって詰むので、クールダウン中に相手が歩ける距離の半分以下に抑えてあります
- **ポーズ（Ver.8）**: 対戦中に `Space` で一時停止。「再開 / タイトルへ」を選べます
- **サマーソルトキック（Ver.9）**: ガイルの**対空技**（`↓` を溜めて `C` を押しながら `↑`）。発生5Fと最速級で、宙返りしながら蹴り上げて2段当たります。ほぼその場で回るので前進はしません（春麗のバードキックは164px進む）。外すと**着地硬直26F**を晒すハイリスク技です。当てると相手を**通常ジャンプの頂点近く（160px）まで巻き上げ**、専用のやられ絵で落ちてきます
- **立ち／しゃがみガードの分離（Ver.9）**: 10キャラ全員に立ちとしゃがみの専用ガード絵を用意しました
- **選択画面のステータスゲージ（Ver.10）**: キャラを選んでいる間、カーソルが乗っているキャラの**威力・リーチ・出の速さ**がゲージで見えます。ザンギエフは威力最大でリーチ最小、ダルシムはその逆、といったキャラ差が選ぶ前に分かります
...
※ Ver.7 からキャラごとに通常技のフレームデータ・威力・リーチ・やられ判定の大きさが違います。固有の必殺技を持つのは今のところ春麗・ダルシム・ガイルで、他7キャラは共通の飛び道具のままです。
...
| キー | バトル中 | メニュー |
| --- | --- | --- |
| ← → | 移動（相手と逆方向でガード。`↓` 併用でしゃがみガード） | カーソル移動 |
| ↑ | ジャンプ | カーソル移動（上の行へ） |
| ↓ | しゃがみ | カーソル移動（下の行へ） |
| Z | パンチ | — |
| X | キック | — |
| C | 必殺技（他7キャラは飛び道具・春麗/ダルシム/ガイルは単押しでは出ない） | — |
| Space | ポーズ（再開 / タイトルへ） | — |
| Enter | ポーズ中の決定 | 決定 |
| Esc | — | 前の画面に戻る |

キャラ固有の必殺技コマンドは操作説明には書きません（技が増えるほど膨れ上がるため）。代わりに**画面の左右・各キャラ側の余白にパネル**が出ます。

| 画面 | パネルの中身 |
| --- | --- |
| キャラ選択 / CPU選択 / ステージ選択 | カーソルが乗っているキャラの**ステータスゲージ**（威力・リーチ・出の速さ）と技のコマンド |
| 対戦 / リザルト | **技表**（技名・コマンド・ダメージ） |

どちらも `CharacterSpec` から組み立てているので、キャラに技を足したり数値を調整しても UI は触らずに反映されます。ゲージは全キャラ中の相対値（毎回 min/max を計算）なので、キャラを追加･調整しても振り切れません。実数値も併記しています。

**「強さ」のような総合値は出しません。** ゲーム内にその数値が存在しないので、作ると実態とずれます。代わりに実データから引ける3軸だけを見せています。

**ラベルに格ゲー用語を使いません。** 「発生」だと初見で意味が取れないので「出の速さ」と書き、各行に実数値（`16` / `68px` / `5F`）とホバー時の補足を添えています。用語が混ざらないことはテストで固定しています。
...
├── scripts/
│   ├── lib/png.mjs                   PNG の読み書きと画素処理（背景除去・面積平均リサンプル）
│   ├── normalize-sprite-sequence.mjs 連番スプライトの正規化（後述）
│   ├── import-pose-images.mjs        単発ポーズの取り込み（余白切り詰め + 長辺512）
│   └── pose-comparison-sheet.mjs     等身を見比べる1枚を出す（描画高を決める道具）
└── src/
...
│                         + UI 用の組み立て（getMoveList / getCharacterStats）
...
│   ├── index.ts          ガイルのスペック
│   ├── cpu.ts            固有の CPU 抽選表
│   └── specials/somersaultKick.ts
└── <他7キャラ>/index.ts + specials/
moves/
...
| 項目 | 意味 |
| --- | --- |
| `sequence` | 再生順。同じ画像を複数回使える（`[0,1,1,1,2,3,4,4]` など） |
| `rotationByStep` | コマごとの回転角（度）。**同じ画像を角度違いで使い回す**ための仕組みで、焼き込まずデータで持つので角度を変えても画像を作り直さなくてよい |
| `airborneOnly` | `true` なら滞空中だけ描く（春麗）。`false` は地上の溜めから着地まで通しで描く（ガイル） |
...
`--contact-sheet` は全コマを重ねて接地線・頭頂線・アンカー線を引いた検証用の1枚です。標準出力に `sprites.ts` へ渡す `height` / `anchorX` / `offsetY` / `pivotY` が出ます。

主なオプション（Ver.9 で追加）:

| オプション | 用途 |
| --- | --- |
| `--remove-background` | 画像の端から同系色を塗りつぶして背景を抜く。**体の内側にある同系色は端から連結していないので守られる**（濃い輪郭線が壁になる）。透過済みの画像は二重処理を避けて自動でスキップ |
| `--anchor centroid` | 足元ではなく不透明画素の重心で揃える。**逆さのコマがある回転技は足元アンカーが使えない**（頭が下に来る） |
| `--scale-by area` | コマごとの頭骨幅ではなく、面積から**全コマ共通の1倍率**を決める |

**`--scale-by area` が要る理由**: 頭骨幅の計測は「頭が画像の上端にある」前提です。サマーソルトキックの蹴り上げコマは上端が脚と衝撃エフェクトなので、実測でそのコマだけ倍率が6倍になりました。素材が同じ縮尺で描かれている場合（背景除去後の面積が ±10% に収まっていれば判断できます）は面積基準の方が安全です。

### 単発ポーズの取り込み（Ver.9）

`scripts/import-pose-images.mjs` は**アルファの外接矩形で余白を切り詰めてから**長辺512pxへ縮小します。

描画側の `drawImageAnchored` は**透明な余白ごと**画像を指定高さに縮めるので、余白があるとキャラが小さく・地面から浮いて描かれます。実測では春麗の立ちガードが180px指定に対し149.7px（17%小さい）・4.4px浮きでした。既存ポーズが余白ゼロなのはそうなっているからです。

```bash
node scripts/import-pose-images.mjs \
  --out-root src/assets/animal-fighter --remove-background \
  "ryu=…/ryu/guard.png:guard" "ryu=…/ryu/crouch-guard.png:crouch-guard" …
```

置き場のファイル名はリポジトリと揃えてあります（`guard.png` = 立ち、`crouch-guard.png` = しゃがみ）。

**描画高の決め方**: ポーズごとの描画高（`getCombatSpriteSpec` の定数）は素材から自動で決められないことが多いので、`scripts/pose-comparison-sheet.mjs` で候補を並べて目視で決めます。

```bash
node scripts/pose-comparison-sheet.mjs --out /tmp/check.png \
  ryu=fight:360 ryu=air-damage:320 ryu=air-damage:360 ryu=air-damage:410
```

実際の描画サイズのまま同じ接地線に並べるので、頭の大きさを見比べれば等身が合っているか分かります（2倍の値を渡すと拡大して見られます）。`air-damage` の 200px はこの方法で決めました。面積比と頭骨幅の2手法がどちらも中央値約208pxを示し、目視で190〜210が妥当だったためです。

計測手法は**ポーズの性質で使えるかが変わります**。頭骨幅は「頭が画像の上端にある」前提なので、腕で顔を覆うガードや、脚を上げるコマでは破綻します。逆にのけぞって頭が露出する `air-damage` では機能しました。新しいポーズを足すときは必ず2手法以上で照合してください。
...
- [x] **Ver.9**: 立ち／しゃがみガードの分離（10キャラ×2枚）、ガイルのサマーソルトキック（対空 / 描画時回転 / 5枚アニメ / 打ち上げ）、空中やられ絵、背景除去と単発ポーズ取り込みのスクリプト
- [x] **Ver.10**: キャラ選択画面のステータスゲージ（威力・リーチ・出の速さ）
- [ ] 地上のやられ絵（`damage.png`）。今は地上で被弾しても棒立ちのまま（空中だけ専用絵がある）
- [ ] 残り7キャラの固有必殺技（波動拳・昇龍拳・スクリューパイルドライバー など）
- [ ] ガードの上下段属性（下段は立ちガードで防げない等。今は立ち／しゃがみで**見た目だけ**が変わる）
