...
- **サマーソルトキック（Ver.9）**: ガイルの**対空技**（`↓` を溜めて `C` を押しながら `↑`）。発生5Fと最速級で、宙返りしながら蹴り上げて2段当たります。ほぼその場で回るので前進はしません（春麗のバードキックは164px進む）。外すと**着地硬直26F**を晒すハイリスク技です
...
└── src/
    ...
    │                                 kick / jump / crouch / guard / crouch-guard / down）
...
- [x] **Ver.9**: 立ち／しゃがみガードの分離（10キャラ×2枚）、ガイルのサマーソルトキック（対空 / 描画時回転 / 5枚アニメ）、背景除去と単発ポーズ取り込みのスクリプト
