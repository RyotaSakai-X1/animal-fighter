...
- **ヨガファイヤー（Ver.8）**: ダルシムの**伸縮する炎**（`↓` を溜めて `C` を押しながら**前**）。弾は飛ばさず、口から伸びて引っ込むまでの炎そのものが当たり判定です。伸びきったところで**343px**（キック95pxの3.6倍）に届きますが、その距離が出るのは6フレームだけ。全体60Fと長いので、外すと手痛く反撃されます。当てると相手を**48px（体1つ分）押し戻し**、炎はその場で引っ込みます（本家の飛び道具が接触で消えるのと同じ）
